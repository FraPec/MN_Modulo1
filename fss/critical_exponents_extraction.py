...
def calculate_nu_gamma(results):
    """Calculate nu and gamma from 1/nu and gamma/nu with error propagation."""
    results['nu'] = 1 / results['1_over_nu']
    results['nu_err'] = results['1_over_nu_err'] / (results['1_over_nu']**2)
    results['gamma'] = results['gamma_over_nu'] * results['nu']
    results['gamma_err'] = results['gamma'] * np.sqrt(
        (results['gamma_over_nu_err'] / results['gamma_over_nu'])**2 +
        (results['nu_err'] / results['nu'])**2
    )
    return results
...
def save_summary_statistics(data, output_dir):
    ...
    summary = {
        'Parameter': ['gamma', 'nu'],
        'Mean': [data['gamma'].mean(), data['nu'].mean()],
        'Std_Dev': [data['gamma'].std(), data['nu'].std()]
    }
...
if __name__ == "__main__":
	...
	L_min_range = [9, 12, 15, 18]
	fit_results = perform_fits(raw_data, L_min_range)
	
	# Calcola nu e gamma con la propagazione degli errori
	final_results = calculate_nu_gamma(fit_results)
	
	# Salva i risultati nel file di output
	save_csv(final_results, OUTPUT_CSV)
	
	# Crea i grafici senza curve di fit
	plot_without_fits(raw_data, final_results, beta_pc_fit_function, chi_max_fit_function, os.path.dirname(INPUT_CSV))
