...
def prepare_dataset_fss_plot(df_means, variable_name, df_vars=None):
    """
    ...
    std_devs_data_set_list = [] if df_vars is not None else None
    means_data_set_list = []
    beta_list = []
    L_list = np.unique(df_means_sorted_b["L"].values)
    ...
    if df_vars is not None:
        ...
        return beta_list, means_data_set_list, L_list
