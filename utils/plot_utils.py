...
def plot_without_fits(data, results, beta_pc_fit_function, chi_max_fit_function, output_dir):
    ...
    plt.ylabel(r"$\beta_{pc}$", fontsize=20)
    plt.title(r"Scatterplot of $\beta_{pc}$ vs $L$", fontsize=20)
    plt.grid()
    ...
    plt.yscale("log")
    plt.xlabel("$L$", fontsize=20)
    plt.ylabel(r"$\chi_{max}'$", fontsize=20)
    plt.title(r"Scatterplot of $\chi_{max}'$ vs $L$", fontsize=20)
...
def plot_critical_exponents(data, output_dir):
    ...
    plt.axhline(y=data['beta_c'].mean(), color='r', linestyle='-', label=f"Mean = {data['beta_c'].mean():.5f}")
    plt.axhline(y=data['beta_c'].mean() + data['beta_c'].std(), color='r', linestyle='--', label=f"Mean + Std = {data['beta_c'].mean():.5f} + {data['beta_c'].std():.5f}")
    plt.axhline(y=data['beta_c'].mean() - data['beta_c'].std(), color='r', linestyle='--', label=f"Mean - Std = {data['beta_c'].mean():.5f} - {data['beta_c'].std():.5f}")
    ...
    plt.axhline(y=data['1_over_nu'].mean(), color='r', linestyle='-', label=f"Mean = {data['1_over_nu'].mean():.5f}")
    plt.axhline(y=data['1_over_nu'].mean() + data['1_over_nu'].std(), color='r', linestyle='--', label=f"Mean + Std  = {data['1_over_nu'].mean():.5f} + {data['1_over_nu'].std():.5f}")
    plt.axhline(y=data['1_over_nu'].mean() - data['1_over_nu'].std(), color='r', linestyle='--', label=f"Mean - Std = {data['1_over_nu'].mean():.5f} - {data['1_over_nu'].std():.5f}")
    plt.xlabel(r"$L_{\mathrm{min}}$")
    plt.ylabel(r"$1/\nu$")
    plt.title(r"$1/\nu$ vs $L_{\mathrm{min}}$")
    ...
    plt.axhline(y=data['gamma_over_nu'].mean(), color='r', linestyle='-', label=f"Mean = {data['gamma_over_nu'].mean():.5f}")
    plt.axhline(y=data['gamma_over_nu'].mean() + data['gamma_over_nu'].std(), color='r', linestyle='--', label=f"Mean + Std = {data['gamma_over_nu'].mean():.5f} + {data['gamma_over_nu'].std():.5f}")
    plt.axhline(y=data['gamma_over_nu'].mean() - data['gamma_over_nu'].std(), color='r', linestyle='--', label=f"Mean - Std = {data['gamma_over_nu'].mean():.5f} - {data['gamma_over_nu'].std():.5f}")
    plt.xlabel(r"$L_{\mathrm{min}}$")
    plt.ylabel(r"$\gamma/\nu$")
    plt.title(r"$\gamma/\nu$ vs $L_{\mathrm{min}}$")
